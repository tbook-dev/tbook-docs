...
- **Queued** — enters the settlement cycle; on settlement the proceeds are **paid directly to the account's wallet** (`transaction.redeem.settled` with `amount`) on `rcusdp-sui`. Vaults that allocate a claimable balance instead signal it via `claimable` in the same event — see the [event catalog](/webhooks/event-types).
- **Instant** — only where [`GET /v1/vaults/:vaultId`](/api-reference/vault-endpoints) reports `settlement.instantRedeem.available: true` with `source: "chain"` (that value is read from the contract per request). Pays USDC immediately, less the fee. Never quote one on `source: "catalog-unverified"`, and keep a queued fallback: the operator switch can flip between your read and the user's signature.
