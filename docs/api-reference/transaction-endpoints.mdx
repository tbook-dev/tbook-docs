...
| `transaction.format` | enum | `sui:tx-bytes-base64` |
...
| `expiresAt` | timestamp | How long we expect you to hold the unsigned bytes: ≈10 min. After expiry, rebuild with a **new** `Idempotency-Key` — the stale intent expires harmlessly |
...
**Body:** `{accountId, recordId}` — `recordId` comes from [`positions.pending.deposits[]`](/api-reference/positions-earnings) — each pending record is cancelled individually.
