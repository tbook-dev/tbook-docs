...
// Sui deactivated its network-wide JSON-RPC service in July 2026 — point
// reads and submits go over gRPC. `SuiClient`/`getFullnodeUrl` no longer
// exist in @mysten/sui 2.x.
...
console.log(res.Transaction.digest);   // e.g. "8vJk…"
```

The response is a discriminated union — `res.Transaction` on success,
`res.FailedTransaction` when execution failed; both carry `digest` and
...
- **Redeem:** `POST /v1/vaults/rcusdp-sui/tx/redeem` — on settlement the proceeds are paid straight to the account's wallet (`transaction.redeem.settled` with `amount`) — see [Transactions](/api-reference/transaction-endpoints).
