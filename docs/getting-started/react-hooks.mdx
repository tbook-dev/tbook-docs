...
Published under the `@tbookdev` npm scope together with `@tbookdev/vault-sdk-sui`; npm publication is pending — until then, install from the monorepo. The [REST contract](/api-reference) is normative.
:::
