...
Account data, webhook management, treasury controls, and analytics always require a **secret** key. Production keys never work on sandbox and vice versa.
...
Responses include `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`; exceeding the limit returns `429` with a `Retry-After` header. Back off and retry — bursts are normal, sustained excess is not.
