...
Redemption queued / proceeds claimable. `confirmed` data includes the locked `shares` (plus `estimatedProceeds` on `rcusdp-sui`); `settled` data includes `claimable` (USDC).
