...
```ts
const BASE = "https://sandbox.rwa-api.tbook.com";
const HEADERS = {
...
const { data: assets } = await fetch(`${BASE}/v1/assets`, { headers: HEADERS })
  .then(r => r.json());
// → rcusdp 8.32% APY (T+1) · xaua gold (T+3)
```
...
const digest = await waas.signAndSubmit(intent.transaction);   // Crossmint, Privy, …
...
**Step 5 — update the UI from webhooks**, not polling: `transaction.deposit.settled` carries `externalId`, `shares`, `sharePrice`.
...
for (const [vaultId, amount] of [["rcusdp-sui", "700000.00"], ["xaua-sui", "300000.00"]]) {
  ...
  await submitToSquadsProposal(intent.transaction);   // your multisig flow
...
The integration is three webhooooks-driven bookkeeping rules (see [Account Models — omnibus](/overview/account-models#omnibus--many-end-users-pooled-in-one-wallet) for the full contract):

```ts
// 1. User "adds to savings" → your ledger debits the user, and you batch-deposit from the pool
// 2. On transaction.deposit.settled → book shares to users pro-rata at event's sharePrice
app.post("/webhooks/tbook", verifyTBookSignature, (req, res) => {
  const event = req.body;
  if (event.type === "transaction.deposit.settled") {
    const { amount, shares, sharePrice } = event.data;
    ledger.allocateShares({ amount, shares, sharePrice });   // YOUR sub-ledger, in shares
  }
  res.sendStatus(200);
});
...
const pool = await fetch(`${BASE}/v1/accounts/${POOL_ACCOUNT_ID}/positions`, { headers: HEADERS })
  .then(r => r.json());
assert(ledger.totalShares() === pool.data[0].shares);   // alert on drift
```

Display each user's balance as `userShares × sharePrice` — yield shows up automatically as the share price rises.
