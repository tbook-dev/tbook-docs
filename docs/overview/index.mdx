...
| **Rail** | How funds reach the asset. TBook vaults use `sui:direct` — the account's USDC is on Sui, the same chain as the asset, so there is no bridge. |
| **Vault** | An investable instance of an asset on a rail: `rcusdp-sui` and `xaua-sui`. The API shape is identical for every vault; only settlement timing differs. |
