...
- [ ] **Compliance attestations wired** — `compliance.kyc.attested` (individual/omnibus) or `compliance.kyb.attested` (business) set before the first deposit; unattested deposits return 403.
...
- [ ] **Error handling reviewed** — `cycle_closed` retry, `expiresAt` rebuild, 409 `vault_paused` handling, idempotency keys on all money movements.
