...
- **Queued (default, all vaults):** `tx/redeem {mode:"queued"}` locks shares into the current cycle → **on settlement the proceeds are paid straight to the account's wallet** — `transaction.redeem.settled` carries the delivered `amount`, and there is nothing further to do. (Verified against a live mainnet settlement cycle: the settle transaction transfers USDC to every redeeming wallet directly.)
