...
SDK packages (`@tbookdev/vault-sdk-sui`, `@tbookdev/vault-node`, `@tbookdev/vault-react-sui`) follow semver independently of the API version; each release documents the API contract version it targets. The packages are currently available from the monorepo (npm publication pending). The legacy Solana packages (`@tbookdev/vault-sdk`, `@tbookdev/vault-react`) are frozen and private.
